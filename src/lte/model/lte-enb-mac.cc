...
void 
EnbMacMemberLteEnbCmacSapProvider::SetRAProcessFlag (bool raProcessContinuing)
{
  //ADDED FOR AN HACK IN NR-GNB-MAC, SHOULD NOT DO ANYTHING
  NS_LOG_FUNCTION (this);
}

void 
EnbMacMemberLteEnbCmacSapProvider::SetCSIRLMResources (uint16_t csiRSPeriodicity, uint16_t csiRSOffset, uint8_t numOfBeamsTbRlm, uint64_t imsi, std::vector<uint8_t> csiCounterVector, bool isServingCellId)
{
  //ADDED FOR CSI-RS SCHEDULING IN 5G-NR, SHOULD NOT DO ANYTHING FOR LTE
  NS_LOG_FUNCTION (this);
}

std::pair<uint8_t, uint8_t>
EnbMacMemberLteEnbCmacSapProvider::GetCSIRSResourceFromMac (uint64_t imsi)
{
  //ADDED FOR CSI-RS SCHEDULING IN 5G-NR, SHOULD NOT DO ANYTHING FOR LTE
  NS_LOG_FUNCTION (this);
}
...
void 
EnbMacMemberLteEnbCmacSapProvider::SetCSIRSFlag (uint64_t imsi, bool csiRSFlag)
{
  //ADDED FOR CSI-RS SCHEDULING IN 5G-NR, SHOULD NOT DO ANYTHING FOR LTE
  NS_LOG_FUNCTION (this);
}
...
void 
EnbMacMemberLteEnbCmacSapProvider::UpdateCSIRLMResources (uint64_t imsi, uint8_t maxCSIperFrame)
{
  //ADDED FOR CSI-RS SCHEDULING IN 5G-NR, SHOULD NOT DO ANYTHING FOR LTE
  NS_LOG_FUNCTION (this);
}
