...
public:
  ...
  uint8_t GetTempCellId ();

  void SetCellToTempCellId (uint8_t tempCellUd);

  uint8_t GetLteCellId ();
  
  void SetLteCellId (uint8_t lteCellId);

  void SetLteEnbRrcSapProviderFromHelper ();

  void DoRecvBeamSweepTriggerFromLTECoordinator ();

  void DoRecvBeamSweepCommandFromLTECoordinator ();

  bool DoIsRrcIdleStart ();

  void DoRecvUeDeRegistrationUpdate (uint64_t imsi);
...
private:
  ...
  void DoSetCellIDTempForBeamTX (uint8_t tempCellId);

  void DoSendUeDeregisterToGnb ();

  bool DoSendCheckForHandoverEvents (uint64_t imsi);

  void DoClearHandoverEventsAtCoordinator ();

  void DoChangeBeamSweepCompleteAtCoordinator (bool beamSweepComplete);
  ...
  /**
   * Nr Version of T310 variable that will be used for detection of a Radio 
   * Link Failure. Different implementations for cancelling Radio Link Failure 
   * process can be considered. For instance, if UE detects an SNR above certain 
   * threshold from a specific beam, than this can process can be cancelled. 
   */
  Time m_nrt310;
  ...
  /**
   * \brief Nr version of the DoNotifyInSync() function
   */
  void DoNotifyOutOfSyncNr ();
  ...
  void StartBeamSweepTimer (bool iaState);

  uint8_t m_tempCellId;
