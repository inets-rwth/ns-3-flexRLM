...
{
  os << "SourceCellId " << m_sourceCellId << " IMSI " << m_ueImsi << " RNTI " << m_ueRnti << " HandoverFlag " << m_handoverFlag;
}
...
bool
EpcX2InformLteCoordinatorHeader::GetHandoverFlag () const
{
  if (m_handoverFlag == 1)
  {
    return true;
  }
  else if (m_handoverFlag == 0)
  {
    return false;
  }
  else
...
void
EpcX2InformLteCoordinatorHeader::SetHandoverFlag (bool handoverFlag)
{
  if (handoverFlag)
  {
    m_handoverFlag = 1;
...
EpcX2UeCSIRSSinrUpdateHeader::EpcX2UeCSIRSSinrUpdateHeader ()
  ...
  EpcX2UeCSIRSSinrUpdateHeader::Serialize (Buffer::Iterator start) const
  {
    ...
    // SINR map
    std::vector<double>::size_type sz = m_sinrMap.size ();
    i.WriteHtonU16 (sz);          // number of elemnents in the map

    for (size_t n = 0; n < m_sinrMap.size (); n++)
    {
      i.WriteHtonU64 (pack754(m_sinrMap.at(n)));
    }

    // Cell Id map
    std::vector<uint8_t>::size_type sy = m_cellIdMap.size();
    i.WriteU8(sy);
    for (int j = 0; j < (int)sy; j++)
    {
      i.WriteU8(m_cellIdMap[j]);
    }

    // Sector map
    std::vector<uint16_t>::size_type sv = m_sectorMap.size();
    i.WriteHtonU16(sv);
    for (int j = 0; j < (int)sv; j++)
    {
      i.WriteHtonU16(m_sectorMap[j]);
    }

    // Elevation map
    std::vector<double>::size_type sw = m_elevationMap.size ();
    i.WriteHtonU16 (sw);          // number of elemnents in the map

    for (size_t n = 0; n < m_elevationMap.size (); n++)
    {
      i.WriteHtonU64 (pack754(m_elevationMap.at(n)));
    }
  }
  ...
  EpcX2UeCSIRSSinrUpdateHeader::Deserialize (Buffer::Iterator start)
  {
    ...
    for (int j = 0; j < sy; j++)
    {
      uint8_t cellId = i.ReadU8();
      m_cellIdMap.emplace_back(cellId);
    }
    m_headerLength += 1 + sy * 1;
    m_numberOfIes += 1 + sy;
    
    // Sector map
    int sv = i.ReadNtohU16();
    for (int k = 0; k < sv; k++)
    {
      uint16_t sector = i.ReadNtohU16();
      m_sectorMap.emplace_back(sector);
    }
    m_headerLength += 2 + sv * 16;
    m_numberOfIes += 1 + sv;

    // Elevation map
    int sw = i.ReadNtohU16();
    for (int j = 0; j < sw; j++)
    {
      double elevation = unpack754(i.ReadNtohU64());
      m_elevationMap.emplace_back(elevation);
    }
    m_headerLength += 2 + sw * 16;
    m_numberOfIes += 1 + sw;
  ...
  EpcX2UeCSIRSSinrUpdateHeader::Print (std::ostream &os) const
  {
    ...
    for (size_t n = 0; n < m_sinrMap.size (); n++)
    {
      os << "Sinr " << 10*std::log10(m_sinrMap.at(n));
    }
  }
  ...
  std::vector<uint8_t>
  EpcX2UeCSIRSSinrUpdateHeader::GetUeCSIRSCellIdMap () const
  {
    return m_cellIdMap;
  }

  void
  EpcX2UeCSIRSSinrUpdateHeader::SetUeCSIRSCellIdMap (std::vector <uint8_t> cellIdMap)
  {
    m_cellIdMap = cellIdMap;
    std::vector<uint8_t>::size_type sz = m_cellIdMap.size ();
    m_headerLength += sz * 1;
    m_numberOfIes += 1;
  }

  std::vector<BeamId>
  EpcX2UeCSIRSSinrUpdateHeader::GetUeCSIRSBeamIdMap() const
  {
    std::vector<BeamId> returnMap;
    for (uint8_t i = 0; i < m_sectorMap.size(); i++)
    {
      returnMap.emplace_back(BeamId(m_sectorMap[i], m_elevationMap[i]));
    }
    return returnMap;
  }

  void
  EpcX2UeCSIRSSinrUpdateHeader::SetUeCSIRSBeamIdMap(std::vector<BeamId> map)
  {
    for (uint8_t i = 0; i < map.size(); i++)
    {
      m_sectorMap.emplace_back(map[i].GetSector());
      m_elevationMap.emplace_back(map[i].GetElevation());
    }

    std::vector<uint16_t>::size_type sy = m_sectorMap.size();
    m_headerLength += sy * 2;
    m_numberOfIes += 1;

    std::vector<double>::size_type sz = m_elevationMap.size();
    m_headerLength += sz * 16;
    m_numberOfIes += 1;
  }
...
//****************************************************************************************************
NS_OBJECT_ENSURE_REGISTERED (EpcX2SuboptimalCSIRSReportHeader);

EpcX2SuboptimalCSIRSReportHeader::EpcX2SuboptimalCSIRSReportHeader ()
  : m_numberOfIes (1 + 1 + 1),
    m_headerLength (8 + 2 + 8)
{
}
...
TypeId
EpcX2SuboptimalCSIRSReportHeader::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::EpcX2SuboptimalCSIRSReportHeader")
    .SetParent<Header> ()
    .SetGroupName("Lte")
    .AddConstructor <EpcX2SuboptimalCSIRSReportHeader> ()
  ;
  return tid;
}

TypeId
EpcX2SuboptimalCSIRSReportHeader::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

uint32_t
EpcX2SuboptimalCSIRSReportHeader::GetSerializedSize (void) const
{
  return m_headerLength;
}

void 
EpcX2SuboptimalCSIRSReportHeader::Serialize (Buffer::Iterator start) const
{
  Buffer::Iterator i = start;

  i.WriteHtonU64 (m_ueImsi);
  i.WriteHtonU16 (m_sector);
  i.WriteHtonU64 (pack754(m_elevation));
}

uint32_t
EpcX2SuboptimalCSIRSReportHeader::Deserialize (Buffer::Iterator start)
{
  Buffer::Iterator i = start;
  ...
  m_sector = i.ReadNtohU16 ();
  m_headerLength = +2;
  m_numberOfIes++;

  m_elevation = unpack754(i.ReadNtohU64());
  m_headerLength = +8;
  m_numberOfIes++;
...
void 
EpcX2SuboptimalCSIRSReportHeader::Print (std::ostream &os) const
{
  os << "UE IMSI " << m_ueImsi << " Sector " << m_sector << " Elevation " << m_elevation;
}

uint64_t 
EpcX2SuboptimalCSIRSReportHeader::GetUeImsi () const
{
  return m_ueImsi;
}

void 
EpcX2SuboptimalCSIRSReportHeader::SetUeImsi (uint64_t ueImsi)
{
  m_ueImsi = ueImsi;
}

uint16_t
EpcX2SuboptimalCSIRSReportHeader::GetSector () const
{
  return m_sector;
}

void 
EpcX2SuboptimalCSIRSReportHeader::SetSector (uint16_t sector) 
{
  m_sector = sector;
}

double 
EpcX2SuboptimalCSIRSReportHeader::GetElevation () const
{
  return m_elevation;
}

void 
EpcX2SuboptimalCSIRSReportHeader::SetElevation (double elevation)
{
  m_elevation = elevation;
}

uint32_t
EpcX2SuboptimalCSIRSReportHeader::GetLengthOfIes () const
{
  return m_headerLength;
}

uint32_t
EpcX2SuboptimalCSIRSReportHeader::GetNumberOfIes () const
{
  return m_numberOfIes;
}

uint64_t
EpcX2SuboptimalCSIRSReportHeader::pack754(long double f)
{
  uint16_t bits = 64;
  uint16_t expbits = 11;
  long double fnorm;
...
long double
EpcX2SuboptimalCSIRSReportHeader::unpack754(uint64_t i)
{
  uint16_t bits = 64;
  uint16_t expbits = 11;
  long double result;
...
//****************************************************************************************************
NS_OBJECT_ENSURE_REGISTERED (EpcX2CSIRSFlagHeader);

EpcX2CSIRSFlagHeader::EpcX2CSIRSFlagHeader ()
  : m_numberOfIes (1 + 1),
    m_headerLength (8 + 2)
{
}
...
TypeId
EpcX2CSIRSFlagHeader::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::EpcX2CSIRSFlagHeader")
    .SetParent<Header> ()
    .SetGroupName("Lte")
    .AddConstructor <EpcX2CSIRSFlagHeader> ()
  ;
  return tid;
}

TypeId
EpcX2CSIRSFlagHeader::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

uint32_t
EpcX2CSIRSFlagHeader::GetSerializedSize (void) const
{
  return m_headerLength;
}

void 
EpcX2CSIRSFlagHeader::Serialize (Buffer::Iterator start) const
{
  Buffer::Iterator i = start;
...
uint32_t
EpcX2CSIRSFlagHeader::Deserialize (Buffer::Iterator start)
{
  Buffer::Iterator i = start;
...
void 
EpcX2CSIRSFlagHeader::Print (std::ostream &os) const
{
  os << "UE IMSI " << m_ueImsi << " Flag " << m_flag;
}

uint64_t 
EpcX2CSIRSFlagHeader::GetUeImsi () const
{
  return m_ueImsi;
}

void 
EpcX2CSIRSFlagHeader::SetUeImsi (uint64_t ueImsi)
{
  m_ueImsi = ueImsi;
}

bool
EpcX2CSIRSFlagHeader::GetFlag () const
{
  if (m_flag == 1)
  {
    return true;
...
void 
EpcX2CSIRSFlagHeader::SetFlag (bool flag) 
{
  if (flag)
  {
    m_flag = 1;
...
uint32_t
EpcX2CSIRSFlagHeader::GetLengthOfIes () const
{
  return m_headerLength;
}

uint32_t
EpcX2CSIRSFlagHeader::GetNumberOfIes () const
{
  return m_numberOfIes;
}
