...
typedef std::vector<std::pair<double, uint8_t>> CellCSIRSSinrMap;
...
public: 
  ...
  Ptr<LteSignalingRadioBearerInfo> GetSignalingRadioBearerInfo (uint8_t srb);
...
public:
  ...
  void SetupUeFromHelper (uint16_t lteRnti, uint64_t imsi);

  std::pair<uint8_t, uint8_t> GetCSIRSResourceVector (uint64_t imsi);

  void DoRecvRRCCSIRSReport (uint64_t imsi, uint16_t cellId, std::vector<double> csiRSVector, std::vector<uint8_t> csiRSCellIdVector, std::vector<BeamId> csiRSBeamIdVector);

  void UpdateCSIRLMResources(uint64_t imsi);

  uint16_t GetLteRnti(uint64_t imsi);

  void SetMinCSIRSFromServingGnb(uint8_t minCSIRS);
...
private:
  ...
  void DoRecvOptimalGnbBeamMap (LteRrcSap::CellOptimalGnbBeamMap msg);

  void DoRecvOptimalGnbBeamReport (LteRrcSap::OptimalGnbBeamReport params);

  void DoRecvTriggerRegisterUe (uint64_t imsi, const Ptr<NetDevice> &netDev);

  void DoRecvRegisterUE (uint64_t imsi);

  void DoRecvDeregisterUe (LteRrcSap::DeRegisterUeContext params);

  void DoRecvClearHandoverEvent (uint64_t imsi);

  void DoRecvBeamPerformComplete (uint64_t imsi, bool beamSweepComplete);

  bool DoCheckForHandoverEvents (uint64_t imsi);

  void DoRecvDeRegisterUeCommand (LteRrcSap::DeRegisterUeContext params); // RECEIVE FUNCTION OF LTE COORDINATOR

  void DoRecvUeDeRegisterCompletedFromGnb (EpcX2SapUser::InformLteCoordinatorParams params); // WILL BE RUN WHEN AN UPDATE HAS BEEN RECEIVED FROM GNB

  void DoSendUeDeRegisterCompletedToLteCo (uint64_t imsi);

  void DoRecvUpdateBeamsTBRLMFromLteCo (LteRrcSap::UpdateBeamsTbRLM params);

  void DoForwardUeSSBRSReport (LteRrcSap::UpdateBeamsTbRLM params);

  void DoRecvSuboptimalCSIRSReport (LteRrcSap::SuboptimalCSIRSReport params);
  ...
  void DoRecvCoordinatorHandoverRequest(EpcX2SapUser::SecondaryHandoverParams);

  void DoRecvInformLteCoordinator(EpcX2SapUser::InformLteCoordinatorParams params);

  void DoRelayRRCCSIRSReport (LteEnbCphySapUser::UeRRCCSIRSReport info);
  ...
  void SetSweepParameters (uint64_t imsi, bool sweepStarted, bool sweepCompleted);
...
private:
  ...
  void InitiateBeamSweepFromRrc (double perceivedSinr, uint64_t imsi); 
  ...
  void BeamSweepTimer (uint64_t imsi);
  ...
  /**
   * The 'BeamSweepCompleteDuration' attribute. After triggering a beam sweep
   * from ENB RRC to UE RRC, if the UE has not started a beam sweep due to 
   * several reasons, boolean m_beamSweepStarted will be set to false via this 
   * timer.
   */
  ...
  HandoverEventMap m_imsiBeamSweepTriggerMap;

  long double m_sinrThresholdDifference;
  double m_maxRateThreshold;

  //TTT HO values needed
  ...
  //Needed to detect larger SINR drops to initiate IA delay
	//NOT USED FOR NOW (part of outage detection in lte-enb-rrc.cc)
  //std::map<uint32_t, long double> prevSinrDb;
  ...
  std::map<uint64_t, CellCSIRSSinrMap> m_imsiCellCSIRSSinrMap;

  //For initializing the TriggerUeAssociationUpdate
	bool m_firstReport;
  ...
  bool m_outageAndHandover {false};

  EventId m_beamSweepTriggered;

  EventId m_sweepParametersChanged;

  //uint8_t m_lteRnti;
  //A map similar to m_imsiRntiMap but it contains the LTE RNTIs.
  //m_imsiRntiMap contains the NR RNTIs
  std::map<uint64_t, uint16_t> m_lteRnti;

  uint16_t m_csiPeriodicityInSlots;
  uint16_t m_csiOffsetInSlots;
  uint8_t m_maxCSIperFrame;

  Time m_ssbPeriodicity;
