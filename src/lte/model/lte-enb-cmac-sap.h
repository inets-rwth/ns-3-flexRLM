...
public:
  ...
  virtual void SetCSIRLMResources (uint16_t csiRSPeriodicity, uint16_t csiRSOffset, uint8_t numOfBeamsTbRlm, uint64_t imsi, std::vector<uint8_t> csiCounterVector, bool isServingCellId) = 0;

  virtual void SetCSIRSFlag (uint64_t imsi, bool csiRSFlag) = 0;

  virtual std::pair<uint8_t, uint8_t> GetCSIRSResourceFromMac (uint64_t imsi) = 0;

  virtual void UpdateCSIRLMResources (uint64_t imsi, uint8_t maxCSIperFrame) = 0;
