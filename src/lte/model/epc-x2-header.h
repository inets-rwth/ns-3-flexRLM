...
public:
  ...
  bool GetHandoverFlag () const;
  void SetHandoverFlag (bool handoverFlag);
...
public:
  ...
  std::vector<uint8_t> GetUeCSIRSCellIdMap () const;
  void SetUeCSIRSCellIdMap (std::vector<uint8_t> cellIdMap);

  std::vector<BeamId> GetUeCSIRSBeamIdMap() const;
  void SetUeCSIRSBeamIdMap(std::vector<BeamId> map);
...
private:
  ...
  std::vector<double> m_sinrMap;
  std::vector<uint8_t> m_cellIdMap;
  std::vector<uint16_t> m_sectorMap;
  std::vector<double> m_elevationMap;
  uint64_t m_ueImsi;
  uint16_t m_sourceCellId; 
};
...
class EpcX2SuboptimalCSIRSReportHeader : public Header
{
public:
  EpcX2SuboptimalCSIRSReportHeader ();
  virtual ~EpcX2SuboptimalCSIRSReportHeader ();
...
class EpcX2CSIRSFlagHeader : public Header
{
public:
  EpcX2CSIRSFlagHeader ();
  virtual ~EpcX2CSIRSFlagHeader ();
