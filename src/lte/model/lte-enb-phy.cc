...
void 
LteEnbPhy::DoSetOptimalGnbBeamForImsi (uint64_t imsi, SfnSf startingSfn, std::vector<uint16_t> optimalBeamIndex, bool isServingCell, uint8_t numOfBeamsTbRlm)
{
  //DoSetOptimalGnbBeamForImsi does not do anything for LteEnbPhy
}

void 
LteEnbPhy::DoAttachUeFromRRC (uint64_t imsi, Ptr<NetDevice> netDev)
{
  //DoRegisterUeFromRRC does not do anything for LteEnbPhy
}

bool
LteEnbPhy::RegisterUe (uint64_t imsi)
{
  return true;
}

void 
LteEnbPhy::DoDeregisterUeFromRRC (uint64_t imsi, uint16_t rnti)
{
  //DoDeregisterUeFromRRC does not do anything for LteEnbPhy
}

void 
LteEnbPhy::SetGnbIAState (bool IAperformed)
{
  // DOES NOTHING FOR LTEENBPHY
}

void 
LteEnbPhy::DoUpdateBeamsTBRLM (uint64_t imsi, std::vector<uint16_t> optimalBeamIndex, SfnSf startingSfn, std::vector<uint8_t> csiCounterVector)
{
  // DOES NOTHING FOR LTEENBPHY
}

void 
LteEnbPhy::DoUpdateSuboptimalCSIRSBeams (uint64_t imsi, uint16_t sector, double elevation)
{
  // DOES NOTHING FOR LTEENBPHY
}
}
