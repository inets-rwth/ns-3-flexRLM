...
/*void 
UeMemberLteUeCmacSapUser::SendOptimalBeamToGnb (SfnSf startingSfn, uint16_t optimalBeamIndex)
{
  m_rrc->DoSendOptimalBeamToGnb (startingSfn, optimalBeamIndex);
}*/
...
LteUeRrc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteUeRrc")
    ...
    .AddAttribute ("NRT310",
                   "Timer for detecting the Radio link failure -NR version",
                   TimeValue (MilliSeconds (10.0)),
                   MakeTimeAccessor (&LteUeRrc::m_nrt310),
                   MakeTimeChecker (MilliSeconds (0), MilliSeconds (2000)))
    .AddAttribute ("N310",
...
LteUeRrc::DoInitialize (void)
{
  ...
  SetLteCellId (11);    //Hard-coded: Lte Enb's cellId is always 11
  ...
  ueParams.firstConnToNetwork = false;
...
{
  NS_LOG_FUNCTION (this);
  Ptr<NrUeNetDevice> nrUeNetDev = m_cphySapProvider.at(0)->GetNetDeviceFromPhy ()->GetObject <NrUeNetDevice> ();
  Ptr<LteUeNetDevice> lteUeNetDev = m_cphySapProvider.at(0)->GetNetDeviceFromPhy ()->GetObject <LteUeNetDevice> ();
      ...
      case CONNECTED_HANDOVER:
        {
          ...
          m_cphySapProvider.at (0)->NotifyHandoverSuccessful ();
        }
        break;
      ...
      case IDLE_CONNECTING:
        {
          ...
          if (m_rlmOn)
          {
            m_cphySapProvider.at(0)->ConfigureCSIRSResources (
                              msg.radioResourceConfigDedicated.csiConfigParams.initialCSIRSResource,
                              msg.radioResourceConfigDedicated.csiConfigParams.csiRSOfset,
                              msg.radioResourceConfigDedicated.csiConfigParams.ciRSPeriodicity,
                              msg.radioResourceConfigDedicated.csiConfigParams.noOfCSIRSResourcesPerSSBurst);
          }
        }
        break;
      ...
      if (m_srb1 == 0)
        {
          ...
          ueParams.firstConnToNetwork = m_firstConnectionToNetwork;
...
LteUeRrc::RadioLinkFailureDetected ()
{
  ...
  if (!m_sweepTriggerFromRrcReceived)
  {
    if (!DoSendCheckForHandoverEvents (m_imsi))
    {
      if (m_cphySapProvider.at(0)->GetLastPerceivedSinr() < m_maxRateThreshold)
      {
        BeamSweepTraceParams params;
        params.imsi = m_imsi;
        params.m_beamSweepOrigin = BeamSweepTraceParams::UE_INITIATED_OUTAGE;
        params.currentCell = m_cellId;
        params.snrBeforeSweep = m_cphySapProvider.at(0)->GetLastPerceivedSinr();
        ...
        if (params.snrBeforeSweep < m_outageThreshold)
        {
          ...
          else if (m_cellId == 0)
          {
            if (DynamicCast<NrUeNetDevice>(m_cphySapProvider.at(0)->GetNetDeviceFromPhy()) != nullptr)
            {
              ...
              {
                NS_LOG_UNCOND("Beam sweep has already been triggered by the UE RRC");
              }
              else
              {
                m_cphySapProvider.at(0)->StartBeamSweep(NrPhy::BeamSweepType::IASweep);
                m_beamSweepStarted = Simulator::Schedule(m_startBeamSweepTimeout, &LteUeRrc::StartBeamSweepTimer, this, false);
              }
            }
          }
        }
        else
        {
          ...
          if (DynamicCast<NrUeNetDevice>(m_cphySapProvider.at(0)->GetNetDeviceFromPhy()) != nullptr)
          {
            ...
            {
              m_cphySapProvider.at(0)->StartBeamSweep(NrPhy::BeamSweepType::BeamTracking);
              m_beamSweepStarted = Simulator::Schedule(m_startBeamSweepTimeout, &LteUeRrc::StartBeamSweepTimer, this, false);
            }
          }
        }
      ...
      {
        NS_LOG_UNCOND("RLF detected previously. No beam sweep has been started as the SINR has recovered to above m_maxRateThreshold dB");
      }
    }
  }
  else
  {
    NS_LOG_UNCOND ("RadioLinkFailureDetected: RLF detected and a handover is possible" <<
      ", a beam sweep for this handover purpose will be carried out");
    if (!DoSendCheckForHandoverEvents (m_imsi))
    {
      BeamSweepTraceParams params;
      params.imsi = m_imsi;
      ...
      if (params.snrBeforeSweep < m_outageThreshold)
      {
        NS_LOG_UNCOND ("A handover is possible but RLF is occurred. Disconnecting from the gNB");
        if (m_cellId != 0)
        {
          DoSendUeDeregisterToGnb ();
        }
...
void 
LteUeRrc::DoNotifyOutOfSyncNr ()
{
  NS_LOG_FUNCTION (this << m_imsi);
  
  if (m_sweepTriggerFromRrcReceived)
  {
    NS_LOG_UNCOND ("RadioLinkFailureDetected: RLF detected, but a handover is scheduled");
    m_cphySapProvider.at(0)->SetPhyIAFlag (false);
  }
  
  if (m_radioLinkFailureDetected.IsRunning ())
  {
    NS_LOG_UNCOND ("t310 started");
  }
  else
  {
...
void 
LteUeRrc::DoSendOptimalBeamMapToLteCoordinator (std::map<uint8_t, std::vector<std::pair<std::pair<SfnSf, uint16_t>, std::pair<double, BeamId>>>> cellOptimalBeamMap)
{
  if (m_beamSweepStarted.IsRunning ())
  {
  ...
  m_cphySapProvider.at(0)->SetPhyIAFlag (false);
  LteRrcSap::CellOptimalGnbBeamMap optimalGnbBeamMsg = LteRrcSap::CellOptimalGnbBeamMap ();
...
void 
LteUeRrc::DoSendUeDeregisterToGnb ()
{
  m_rrcSapUser->SendDeregisterUe (GetImsi ());
}

bool 
LteUeRrc::DoSendCheckForHandoverEvents (uint64_t imsi)
{
  return m_rrcSapUser->SendCheckForHandoverEvents (imsi);
}

void 
LteUeRrc::DoSetCellIDTempForBeamTX (uint8_t tempCellId)
{
  m_tempCellId = tempCellId;
}

void 
LteUeRrc::DoClearHandoverEventsAtCoordinator ()
{
  m_rrcSapUser->SendClearHandoverEvents (GetImsi ());
}

void 
LteUeRrc::DoChangeBeamSweepCompleteAtCoordinator (bool beamSweepComplete)
{
  m_rrcSapUser->SendBeamPerformComplete (GetImsi (), beamSweepComplete);
}

uint8_t
LteUeRrc::GetTempCellId ()
{
  return m_tempCellId;
}

void 
LteUeRrc::SetCellToTempCellId (uint8_t tempCellId)
{
  m_cellId = tempCellId;
}

uint8_t
LteUeRrc::GetLteCellId ()
{
  return m_lteCellId;
}

void 
LteUeRrc::SetLteCellId (uint8_t lteCellId)
{
  m_lteCellId = lteCellId;
}


void
LteUeRrc::LeaveConnectedModeNr ()
{
  NS_LOG_FUNCTION (this << m_imsi);
  m_leaveConnectedMode = true;
  m_storedMeasValues.clear ();
...
void 
LteUeRrc::SetLteEnbRrcSapProviderFromHelper ()
{
  m_rrcSapUser->DoSetLteEnbRrcSapProviderFromHelper ();
}

void 
LteUeRrc::DoRecvBeamSweepTriggerFromLTECoordinator ()
{
  m_sweepTriggerFromRrcReceived = true;
  Simulator::Schedule (m_beamSweepTimeoutDuration, &LteUeRrc::BeamSweepTimer, this);
}

void 
LteUeRrc::DoRecvBeamSweepCommandFromLTECoordinator ()
{
  if (DynamicCast<NrUeNetDevice> (m_cphySapProvider.at(0)->GetNetDeviceFromPhy ()) != nullptr)
    {
      if (m_beamSweepStarted.IsRunning ())
...
void 
LteUeRrc::DoRecvUeDeRegistrationUpdate (uint64_t imsi)
{
  if (DynamicCast<NrUeNetDevice> (m_cphySapProvider.at(0)->GetNetDeviceFromPhy ()) != nullptr)
    {
      if (m_beamSweepStarted.IsRunning ())
    ...
    m_cphySapProvider.at(0)->CancelPendingReceivePduEvents ();
...
void 
LteUeRrc::DoSendSSBRSBeamReport (std::vector<std::pair<uint8_t, uint16_t>> optimalBeamIndex, std::map<uint8_t, SfnSf> mapOfStartingSfn, uint8_t servingCellId, uint8_t noOfBeamsTbReported)
{
...
void
LteUeRrc::StartBeamSweepTimer (bool iaState)
{
  m_cphySapProvider.at(0)->SetPhyIAFlag (false);
  m_cphySapProvider.at(0)->SetPhyIAState (false);
}
