...
public:
  ...
  virtual void DeregisterUeFromRRC (uint64_t imsi, uint16_t rnti) = 0;

  virtual void SetGnbIAFlagFromRRC (bool iaState) = 0;

  virtual void UpdateBeamsTBRLM (uint64_t imsi, std::vector<uint16_t> optimalBeamIndexVector, SfnSf startingSfn, std::vector<uint8_t> csiCounterVector) = 0;
...
public:
  ...
  virtual void DeregisterUeFromRRC (uint64_t imsi, uint16_t rnti);
...
template <class C>
void
MemberLteEnbCphySapProvider<C>::SetOptimalGnbBeamForImsi (uint64_t imsi, SfnSf startingSfn, std::vector<uint16_t> optimalBeamIndex, bool isServingCell, uint8_t numOfBeamsTbRlm)
{
...
template <class C>
void 
MemberLteEnbCphySapProvider<C>::DeregisterUeFromRRC (uint64_t imsi, uint16_t rnti)
{
  m_owner->DoDeregisterUeFromRRC (imsi, rnti);
}
...
template <class C>
void 
MemberLteEnbCphySapProvider<C>::UpdateBeamsTBRLM (uint64_t imsi, std::vector<uint16_t> optimalBeamIndexVector, SfnSf startingSfn, std::vector<uint8_t> csiCounterVector)
{
  m_owner->DoUpdateBeamsTBRLM (imsi, optimalBeamIndexVector, startingSfn, csiCounterVector);
}

template <class C>
void 
MemberLteEnbCphySapProvider<C>::UpdateSuboptimalCSIRSBeams (uint64_t imsi, uint16_t sector, double elevation)
{
