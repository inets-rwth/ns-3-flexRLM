...
public:
  ...
  /**
   * \brief Attach UE to the LTE Coordinator enbNetDevice
   */
  void AttachToLteCoordinator (const Ptr<NetDevice> &ueDevice, const Ptr<NetDevice> &gnbDevice);
  ...
  */
  ...
  void RayTraceModelLoadData (NetDeviceContainer ueNetDev, OperationBandInfo *band, std::vector<uint16_t> walkId, std::string input_raytracing_folder);

  void AddBandwidthPart (uint32_t id, const BandwidthPartInfoPtr&bwpInfo);
...
private:
  ...
  void EnableRadioLinkMonitoringTrace ();
  ...
  std::unordered_map<uint32_t, BandwidthPartInfo> m_bwpConfiguration;
  ...
  //!< A map of maps of maps. First key denotes the CC key, the second denotes bwp Id.
  std::map<uint8_t,std::map<uint8_t, Ptr<ThreeGppSpectrumPropagationLossModel > > > m_raytracing;
