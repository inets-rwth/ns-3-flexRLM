                                 ...
                                 const Ptr<NrUeNetDevice>& ueDev) const
{
  for (uint8_t ccId = 0; ccId < gNbDev->GetCcMapSize () ; ccId++)
    {
      //double max = 0; variable not used
      ...
      gNbPhy->GetBeamManager ()->ChangeBeamformingVector (ueDev);
...
double
IdealBeamformingHelper::GetBeamSearchAngleStep () const
{
  Ptr<CellScanBeamforming> cellScanBeamformingAlgorithm = DynamicCast<CellScanBeamforming> (m_idealBeamformingAlgorithm);
  if (cellScanBeamformingAlgorithm != nullptr)
  {
    return cellScanBeamformingAlgorithm->GetBeamSearchAngleStep();
  }
  else
  {
    return 7.5;
