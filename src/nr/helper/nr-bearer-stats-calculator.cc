  ...
  m_pendingOutput (false)
{
  ...
  //variables listed below will be initialized during first function call of DlRxPdu
  //timeLastOutput;
  //totalBytesReceived;
  //timeLastOutputRlc;
  //totalBytesReceived;
...
NrBearerStatsCalculator::GetTypeId (void)
{
  static TypeId tid =
    ...
    .AddAttribute ("UlPdcpOutputFilename",
                   ...
                   MakeStringChecker ())
    .AddAttribute ("DlPdcpThruFilename",
                   "Name of the file where the downlink results will be saved.",
                   StringValue ("DlPdcpThroughput.txt"),
                   MakeStringAccessor (&NrBearerStatsCalculator::m_dlPdcpThruFilename),
                   MakeStringChecker ())
    .AddAttribute ("DlRlcThruFilename",
                   "Name of the file where the downlink results will be saved.",
                   StringValue ("DlRlcThroughput.txt"),
                   MakeStringAccessor (&NrBearerStatsCalculator::m_dlRlcThruFilename),
                   MakeStringChecker ())
  ;
  return tid;
}
...
NrBearerStatsCalculator::DlTxPdu (uint16_t cellId, uint64_t imsi, uint16_t rnti, uint8_t lcid, uint32_t packetSize)
{
  ...
  m_dlOutFile << "DlTxPDU " << Simulator::Now () << " " << cellId << " "
              << rnti << " " << imsi << " " << (uint32_t) lcid << " " << packetSize << " " << std::endl;
...
NrBearerStatsCalculator::DlRxPdu (uint16_t cellId, uint64_t imsi, uint16_t rnti, uint8_t lcid, uint32_t packetSize, uint64_t delay)
{
  ...
  if (timeLastOutput.find(imsi) == timeLastOutput.end()) {
    //this is the first time calling this function for imsi
    //initialise some data structures
    totalBytesReceived.insert(std::pair<uint64_t,uint32_t>(imsi, 0));
    timeLastOutput.insert(std::pair<uint64_t,double>(imsi, 0));
    totalBytesReceivedRlc.insert(std::pair<uint64_t,uint32_t>(imsi, 0));
    timeLastOutputRlc.insert(std::pair<uint64_t,double>(imsi, 0));
  }
  ...
  m_dlOutFile << "DlRxPDU " << Simulator::Now () << " " << cellId << " "
              << rnti << " " << imsi << " " << (uint32_t) lcid << " " << packetSize << " " << delay << std::endl;


  if (m_protocolType == "PDCP")
  {
    totalBytesReceived.at(imsi) += packetSize;
    if (((Simulator::Now ().GetSeconds () - timeLastOutput.at(imsi)) > 0.05))
    {
      if (!m_dlPdcpThruFile.is_open ())
      {
        m_dlPdcpThruFile.open (m_dlPdcpThruFilename.c_str ());
      }

      m_dlPdcpThruFile << Simulator::Now ().GetSeconds () << " " << cellId << " " << rnti << " " << imsi << " "
                           << (uint32_t) lcid << " " << totalBytesReceived.at(imsi) * 8 / (Simulator::Now ().GetSeconds ()- timeLastOutput.at(imsi)) << " " << std::endl;

      timeLastOutput.at(imsi) = Simulator::Now().GetSeconds();
      totalBytesReceived.at(imsi) = 0;
    }
  }
  else if (m_protocolType == "RLC")
  {
    // NS_LOG_UNCOND("rlc");
    totalBytesReceivedRlc.at(imsi) += packetSize;
    if (((Simulator::Now().GetSeconds() - timeLastOutputRlc.at(imsi)) > 0.5))
    {
      if (!m_dlRlcThruFile.is_open())
      {
        m_dlRlcThruFile.open(m_dlRlcThruFilename.c_str());
      }

      m_dlRlcThruFile << Simulator::Now().GetSeconds() << " " << cellId << " " << rnti << " " << imsi << " "
                      << (uint32_t)lcid << " " << totalBytesReceivedRlc.at(imsi) * 8 / (Simulator::Now().GetSeconds() - timeLastOutputRlc.at(imsi)) << " " << std::endl;

      timeLastOutputRlc.at(imsi) = Simulator::Now().GetSeconds();
      totalBytesReceivedRlc.at(imsi) = 0;
    }
  }
