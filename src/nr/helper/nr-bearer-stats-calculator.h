 ...
 */
class NrBearerStatsCalculator : public LteStatsCalculator
{
public:
  /**
   * Class constructor
...
private:
  ...
  std::map<uint64_t, uint32_t> totalBytesReceived;

  std::string m_dlRlcThruFilename;
  std::ofstream m_dlRlcThruFile;
  std::map<uint64_t, double> timeLastOutputRlc;
  std::map<uint64_t, uint32_t> totalBytesReceivedRlc;
};
