...
NrHelper::GetTypeId (void)
{
  static TypeId
    ...
    .AddAttribute ("LoadBalancing",
                   "Boolean which indicates whether load balancing will be performed",
                   BooleanValue (false),
                   MakeBooleanAccessor (&NrHelper::m_loadBalancing),
                   MakeBooleanChecker ())
    .AddAttribute ("MinCSIRSFromServingGnb",
                   "Minimum number of CSI-RS beams from the serving gNB to be used for the RLM procedure",
                   UintegerValue (4),
                   MakeUintegerAccessor (&NrHelper::m_minCSIRSFromServingGnb),
                                      ...
                                      const std::vector<std::reference_wrapper<BandwidthPartInfoPtr> > allBwps)
{
  ...
  rrc->SetMinCSIRSFromServingGnb(m_minCSIRSFromServingGnb);
...
NrHelper::InstallSingleLteEnbDevice (Ptr<Node> n)
{
  ...
  rrc->m_realisticIA = m_realisticIA;
  rrc->m_rlmOn = m_rlmOn;
  rrc->m_loadBalancing = m_loadBalancing;
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  if (!m_realisticIA)  ///IF realistic IA is employed, dont Attach to the ENB
    {
                                    ...
                                    const Ptr<NetDevice> &enbDevice)
{
  ...
  uint16_t lteRnti = enbNetDev->GetRrc ()->AddUe (UeManager::CONNECTION_LTE_COORDINATOR, 0);
  enbNetDev->GetRrc ()->SetupUeFromHelper (lteRnti, ueNetDev->GetImsi());
...
void
NrHelper::EnableRadioLinkMonitoringTrace (void)
{
  Config::Connect ("/NodeList/*/DeviceList/*/ComponentCarrierMapUe/*/NrUePhy/RadioLinkMonitoringTrace",
                   MakeBoundCallback (&NrPhyRxTrace::RadioLinkMonitoringTraceCallback, m_phyStats));
  Config::Connect ("/NodeList/*/DeviceList/*/LteEnbRrc/RadioLinkMonitoringTrace",
                   MakeBoundCallback (&NrPhyRxTrace::RadioLinkMonitoringTraceCallback, m_phyStats));
}
...
void NrHelper::RayTraceModelLoadData(NetDeviceContainer ueNetDev, OperationBandInfo *band, std::vector<uint16_t> walkId, std::string input_raytracing_folder)
{
  for (const auto &cc : band->m_cc)
  {
    for (const auto &bwp : cc->m_bwp)
    {
      m_raytracing[cc->m_ccId][bwp->m_bwpId]->LoadAllTraceData(ueNetDev, walkId, input_raytracing_folder);
    }
  }
}
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
