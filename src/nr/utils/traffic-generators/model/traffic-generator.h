...
class TrafficGenerator : public Application
{
  ...
  private:
    ...
    void logTGDrop(int size);

    static std::ofstream m_TGDropFile;
    static std::string m_TGDropFileName;

    // inherited from Application base class.
    void StartApplication() override; // Called at time specified by Start

    // N nötig um Packets zu senden, die UDP Datagram size übersteigen
    void SendSplitPacket(uint32_t remainingSize, uint32_t iteration);
    /**
