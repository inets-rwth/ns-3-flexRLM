...
NrGnbPhy::DlCtrl (const std::shared_ptr<DciInfoElementTdma> &dci)
{
  ...
  if (m_realisticIA)
  {
    ...
    if (m_rlmOn && !IAisPerformed)
    {
      ...
      if (boolImsiIndexPair.first.first && !boolImsiIndexPair.first.second && m_currSymStart == 1)
      {
        if (m_ctrlMsgs.size() == 0)
        {
  ...
  if (m_ctrlMsgs.size () > 0)
    {
      ...
      if (m_deviceMap.size () != 0 || m_minCSIRSFromServingGnb < 4) // latter: send CSI-RS to UEs not connected to this gNB if strategy is enabled
      {
        if (csiReceived && boolImsiIndexPair.first.first)
        {
          ...
          Simulator::Schedule (GetSymbolPeriod () * 2 - NanoSeconds (1.0),
                              ...
                              m_beamManager,
                              m_ueAttachedImsiMap.at(boolImsiIndexPair.second.first));
        }
      }
                                ...
                                const std::shared_ptr<DciInfoElementTdma> &dci)
{
  ...
  uint64_t dstImsi = m_enbCphySapUser->GetImsiFromRnti (dci->m_rnti);
  std::list<Ptr<NrControlMessage> > ctrlMsgs;
  m_spectrumPhy->StartTxDataFrames (pb, ctrlMsgs, varTtiPeriod, dci->m_symStart, dstImsi);
}
...
NrGnbPhy::PhyCtrlMessagesReceived (const Ptr<NrControlMessage> &msg)
{
  ...
  else if (msg->GetMessageType () == NrControlMessage::CSI_REPORT)
  {
        ...
        m_ueAttachedRnti.find (csiReportMsg->GetRnti ()) != m_ueAttachedRnti.end ())
    {
      ...
      std::vector<std::pair<uint8_t, std::pair<double, BeamId>>> csiReportForRRC;
      auto csiRSSnr = 0.0;
      uint8_t csiCellId;
      for (auto n = 0; n < m_noOfBeamsTbReported; n++)
      {
        csiRSSnr = csiReportMsg->GetCSIRSMap (n).second.second;
        csiResourceIndex = csiReportMsg->GetCSIRSMap (n).first.first;
        csiCellId = csiReportMsg->GetCSIRSMap (n).first.second;
        m_csiRSTbReported.at(imsiOfCSI).emplace_back (std::pair<std::pair<SfnSfKey, uint8_t>, std::pair<BeamId, double>> (
                          {csiResourceIndex, csiCellId}, 
                          {csiReportMsg->GetCSIRSMap (n).second.first, csiRSSnr}));
        
        csiReportForRRC.emplace_back (std::make_pair(csiCellId, std::make_pair(csiRSSnr, csiReportMsg->GetCSIRSMap (n).second.first)));
      }
      ...
      {
        BeamId maxSNRBeamId = m_csiRSTbReported.at(imsiOfCSI).at (0).second.first;
        uint8_t maxSNRCellId = m_csiRSTbReported.at(imsiOfCSI).at (0).first.second;

        if (maxSNRCellId != GetCellId())
        {
          maxSNRBeamId = m_beamManager->GetBeamId (m_ueAttachedImsiMap.at(imsiOfCSI));
          // this can happen for multi gNB CSI-RS RLM
          // best CSI-RS beam is from another gNB than this one. Possibly, a handover is initiated at the RRC
          // in the meantime, check whether one of the other CSI-RS beams (from this cell) can provide a better signal
          for (uint8_t csiIter = 1; csiIter < m_csiRSTbReported.at(imsiOfCSI).size(); csiIter++)
          {
            if (m_csiRSTbReported.at(imsiOfCSI).at(csiIter).first.second == GetCellId())
            {
              maxSNRBeamId = m_csiRSTbReported.at(imsiOfCSI).at(csiIter).second.first;
              break;
            }
          }
        }
...
void 
NrGnbPhy::DoUpdateBeamsTBRLM (uint64_t imsi, std::vector<uint16_t> optimalBeamIndexVector, SfnSf startingSfn, std::vector<uint8_t> csiCounterVector)
{
  uint16_t actualIndex;
  ...
  if (csiCounterVector.size() == 1 && csiCounterVector[0] > m_noOfBeamsTbRLM)
  {
  ...
  for (size_t beamIterator = 0; beamIterator < optimalBeamIndexVector.size (); beamIterator++)
  {
    actualIndex = ((startingSfn.GetSubframe() * startingSfn.GetSlotPerSubframe() * 2) + (startingSfn.GetSlot() * 2) + (optimalBeamIndexVector.at(beamIterator) - 1)) % m_beamVectorMapGnb.size();
    tmp_beamsTbRLM.push_back(std::make_pair(GetCellId(), m_beamVectorMapGnb.at(actualIndex)));
  }
...
void NrGnbPhy::DoUpdateSuboptimalCSIRSBeams(uint64_t imsi, uint16_t sector, double elevation)
{
  ...
  BeamformingVector maxCsiBfv = BeamformingVector(
      CreateDirectionalBfv(m_beamManager->GetAntennaArray(),
                           maxCSIRSBeamId.GetSector(),
                           maxCSIRSBeamId.GetElevation()),
      maxCSIRSBeamId);
  m_beamManager->SaveBeamformingVector(maxCsiBfv, m_ueAttachedImsiMap.at(imsi));
}
}
