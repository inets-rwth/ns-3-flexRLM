...
public:
  ...
  virtual void SetCSIRLMResources (uint16_t csiRSPeriodicity, uint16_t csiRSOffset, uint8_t numOfBeamsTbRlm, uint64_t imsi, std::vector<uint8_t> csiCounterVector, bool isServingCellId);
  virtual std::pair<uint8_t, uint8_t> GetCSIRSResourceFromMac (uint64_t imsi);
  virtual void SetCSIRSFlag (uint64_t imsi, bool csiRSFlag);
  virtual void UpdateCSIRLMResources (uint64_t imsi, uint8_t maxCSIperFrame);
private:
  NrGnbMac* m_mac;
};
...
void 
NrGnbMacMemberEnbCmacSapProvider::SetCSIRLMResources (uint16_t csiRSPeriodicity, uint16_t csiRSOffset, uint8_t numOfBeamsTbRlm, uint64_t imsi, std::vector<uint8_t> csiCounterVector, bool isServingCellId)
{
  m_mac->DoSetCSIRSRLMResources (csiRSPeriodicity, csiRSOffset, numOfBeamsTbRlm, imsi, csiCounterVector, isServingCellId);
}
...
void
NrGnbMacMemberEnbCmacSapProvider::SetCSIRSFlag (uint64_t imsi, bool csiRSFlag)
{
  m_mac->DoSetCSIRSFlag (imsi, csiRSFlag);
}

void 
NrGnbMacMemberEnbCmacSapProvider::UpdateCSIRLMResources (uint64_t imsi, uint8_t maxCSIperFrame)
{
  m_mac->DoUpdateCSIRSRLMResources (imsi, maxCSIperFrame);
}
...
public:
  ...
  std::pair<std::pair<bool, bool>, std::pair<uint64_t, uint8_t>> IsCSIRSRequired (const SfnSf &dlSfn) override;
...
void
NrMacEnbMemberPhySapUser::ForwarIASStateToSched (bool iaPerformed, uint64_t imsi)
{
  m_mac->DoForwardIAStateToSched (iaPerformed, imsi);
}

std::pair<std::pair<bool, bool>, std::pair<uint64_t, uint8_t>>
...
NrGnbMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrGnbMac")
    ...
    .AddAttribute ("NumberOfBeamsTbRLM",
                   ...
                   MakeUintegerChecker<uint8_t> ())
    .AddAttribute ("MinCSIRSFromServingGnb",
                   "Minimum number of CSI-RS beams from the serving gNB to be used for the RLM procedure",
                   UintegerValue (4),
                   MakeUintegerAccessor (&NrGnbMac::m_minCSIRSFromServingGnb),
...
NrGnbMac::SendRar (const std::vector <BuildRarListElement_s> &rarList)
{
  ...
  uint16_t raRnti = 1; // NO!! 38.321-5.1.3 //m_currentSlot is known
...
NrGnbMac::DoRemoveUe (uint16_t rnti)
{
  ...
  else if (m_minCSIRSFromServingGnb < 4)
  {
    // Do not delete CSI-RS resources from gNB after UE handed over to another gNB.
    // This enables the gNB to continue CSI-RS transmission after quick handover.
    // Clearing m_mapOfCSIRSResources after beam sweep and handover should be handled via LteEnbRrc::DoRecvOptimalGnbBeamReport
    // Here: remove placeholders as this gNB is no longer the serving gNB!

    std::map<uint64_t,std::map<uint8_t, std::vector<std::pair<std::pair<uint8_t, uint8_t>, bool>>>>::iterator imsiIt = m_mapOfCSIRSResources.find (imsi);
    if (imsiIt != m_mapOfCSIRSResources.end())
    {
      std::map<uint8_t, std::vector<std::pair<std::pair<uint8_t, uint8_t>, bool>>>::iterator frameIt;
      for(frameIt = m_mapOfCSIRSResources.at(imsi).begin(); frameIt != m_mapOfCSIRSResources.at(imsi).end(); ++frameIt)
      {     
        std::vector<std::pair<std::pair<uint8_t, uint8_t>, bool>>::iterator vectorIt;
        for (vectorIt = (*frameIt).second.begin(); vectorIt != (*frameIt).second.end();)
        {
          if (vectorIt->second == true)
          {
            vectorIt = frameIt->second.erase(vectorIt);
          }
          else
          {
            ++vectorIt;
        ...
        if (frameIt->second.size() == 0)
        {
          // erase frame map element if it is empty
          imsiIt->second.erase(frameIt);
        }
      }
      if (imsiIt->second.size() == 0)
      {
        // erase frame map element if it is empty
        m_mapOfCSIRSResources.erase(imsi);
      }
...
void
NrGnbMac::DoSetCSIRSRLMResources (uint16_t csiRSPeriodicity, uint16_t csiRSOffset, uint8_t numOfBeamsTbRlm, uint64_t imsi, std::vector<uint8_t> csiCounterVector, bool isServingCellId)
{
  // numOfBeamsTbRlm specifies how many CSI-RSs are sent from this gNB to the UE
  // csiCounterVector indicates the position of those CSI-RSs in the time domain -> csiRSOffset + imsi + csiCounterVector[n]*(csiRSPeriodicity + 1) - 1
  // If this is the serving gNB and multi gNB RLM is enabled, numOfBeamsTbRlm can be greater than csiCounterVector.size()
  // -> the scheduler of the serving gNB needs to be aware of another gNB transmitting to the UE in that slot

  //value 8 below is for numerology=3, it should be made generic
  NS_ABORT_MSG_IF (2 * csiRSOffset + csiRSPeriodicity * (numOfBeamsTbRlm - 1) + numOfBeamsTbRlm > 16 * 8,
  ...
  if (!isServingCellId && csiCounterVector.size() == 1 && csiCounterVector[0] > m_noOfBeamsTbRLM)
  {
    // This should be called when a beam from this gNB was previously monitored by that IMSI.
    // Now this is no longer being monitored and the resources should be freed.
    return;
  }
  else if (!isServingCellId && csiCounterVector.size() == 0)
  {
    // similar as above but this is call is triggered by a beam sweep (most likely after RLF of the UE)
    return;
  }

  if (isServingCellId && numOfBeamsTbRlm < m_noOfBeamsTbRLM)
  {
    // overwrite if this is the serving gNB to signal busy slot to scheduler
    // in that slot, another gNB transmits to the UE
    numOfBeamsTbRlm = m_noOfBeamsTbRLM;
  }

  for (auto n = 0; n < numOfBeamsTbRlm; n++)
  {
    std::vector<std::pair<std::pair<uint8_t, uint8_t>, bool>> vectorOfCsiRSResourceSlot;
    ...
    {
      csiCounter = n;
      if (std::find(csiCounterVector.begin(), csiCounterVector.end(), n) == csiCounterVector.end())
      {
        isPlaceholder = true;
      }
    }
    ...
    {
      NS_ASSERT(csiCounterVector.size() == numOfBeamsTbRlm);
      csiCounter = csiCounterVector[n];
    }

    if (n == 0)
    {
      csiRSResourceSlot = lastSSBSlot.GetFutureSfnSf (csiRSOffset + imsi + csiCounter*(csiRSPeriodicity + 1) - 1);
      frameRemainder = csiRSResourceSlot.GetFrame () % (uint8_t)2;
      
      vectorOfCsiRSResourceSlot.push_back ({{csiRSResourceSlot.GetSubframe (), csiRSResourceSlot.GetSlot ()}, isPlaceholder});
      std::map<uint8_t, std::vector<std::pair<std::pair<uint8_t, uint8_t>, bool>>> mapOfCsiRSResourceSlot;
    ...
    else
    {
      ...
      {
        std::pair<uint8_t, uint8_t> resourcePair = std::make_pair(csiRSResourceSlot.GetSubframe (), csiRSResourceSlot.GetSlot ());
        m_mapOfCSIRSResources.at (imsi).at(frameRemainder).push_back(make_pair(resourcePair, isPlaceholder));
      }
      else
      {
        vectorOfCsiRSResourceSlot.push_back ({{csiRSResourceSlot.GetSubframe (), csiRSResourceSlot.GetSlot ()}, isPlaceholder});
...
NrGnbMac::DoGetInitialCSIRSResource (uint64_t imsi)
{
  ...
  {
    return m_mapOfCSIRSResources.at(imsi).begin()->second.at(0).first;
  }
}
...
{
  if (m_realisticIA && m_rlmON & !m_csiRSTimer)
  {
    auto currentDlFn = dlSfn.GetFrame ();     //Frame Number
    auto currentDlSfn = dlSfn.GetSubframe (); //Subframe Number
    auto currentDlSn = dlSfn.GetSlot ();      //Slot Number
    
    for (std::map<uint64_t, std::map<uint8_t, std::vector<std::pair<std::pair<uint8_t, uint8_t>, bool>>>>::iterator csiRsMapImsiIter = m_mapOfCSIRSResources.begin ();
                                                                              csiRsMapImsiIter != m_mapOfCSIRSResources.end ();
                                                                              ++csiRsMapImsiIter)
    {
      uint8_t rlmIndex = 0;
      if (!(currentDlFn % (uint8_t)2 == 0 && currentDlSfn < 4) && !m_csiRSFlag.at((uint64_t)(csiRsMapImsiIter->first)))
      {
        for (uint8_t frameRemainder = 0; frameRemainder < 2; frameRemainder++)
        {
          if (csiRsMapImsiIter->second.find(frameRemainder) != csiRsMapImsiIter->second.end())
          {
            for (std::vector<std::pair<std::pair<uint8_t, uint8_t>, bool>>::iterator csiRSIter = csiRsMapImsiIter->second.at(frameRemainder).begin();
                 csiRSIter != csiRsMapImsiIter->second.at(frameRemainder).end();
                 ++csiRSIter)
            {
              if (currentDlSfn == csiRSIter->first.first && currentDlSn == csiRSIter->first.second && frameRemainder == currentDlFn % (uint8_t)2)
              {
                return {{true, csiRSIter->second}, {(uint64_t)(csiRsMapImsiIter->first), (uint8_t)rlmIndex}};
              }
              else if(!csiRSIter->second)
              {
                rlmIndex++;
                continue;
              }
    ...
    return {{false, false}, {(uint64_t)0, 0}}; //If current SfnSf does not correspond to any resource listed in m_listOfCSIRSResources,
                  //it means that time resource is not assigned CSI RS resource
                  // also if m_csiRSFlag for current IMSI is false
  }
  else
  {
    return {{false, false}, {(uint64_t)0, 0}};
  }
  
}

void 
NrGnbMac::DoSetCSIRSFlag (uint64_t imsi, bool csiRSFlag)
{
  if(m_csiRSFlag.find(imsi)==m_csiRSFlag.end()){
    // imsi not found in map
    m_csiRSFlag.insert({imsi, csiRSFlag});
  }else{
    m_csiRSFlag.at(imsi) = csiRSFlag;
  }  
}

void
NrGnbMac::DoUpdateCSIRSRLMResources (uint64_t imsi, uint8_t maxCSIperFrame)
{
  // After handover to a gNB from which CSI-RS beams are already monitored, 
  // we have to set the placeholders in m_mapOfCSIRSResources to enable other gNBs to continue transmitting CSI-RSs in those slots.
  auto lastSSBSlot = SfnSf (0, 4, 0, 3);
  SfnSf csiRSResourceSlot;
  uint8_t frameRemainder;
  ...
  for (auto n = 0; n < m_noOfBeamsTbRLM; n++)
  {
    ...
    csiRSResourceSlot = lastSSBSlot.GetFutureSfnSf (m_csiOffsetInSlots + imsi + n*(m_csiPeriodicityInSlots + 1) - 1);
    frameRemainder = csiRSResourceSlot.GetFrame () % (uint8_t)2;

    std::map<uint8_t, std::vector<std::pair<std::pair<uint8_t, uint8_t>, bool>>>::iterator frameIt = m_mapOfCSIRSResources.at(imsi).find(frameRemainder);
    if (frameIt != m_mapOfCSIRSResources.at(imsi).end())
    {
      std::pair<uint8_t, uint8_t> pairToFind = {csiRSResourceSlot.GetSubframe (), csiRSResourceSlot.GetSlot ()};

      auto sfnSfIt = std::find_if( frameIt->second.begin(), frameIt->second.end(),[&pairToFind](const std::pair<std::pair<uint8_t, uint8_t>, bool>& element){ return element.first == pairToFind;} );
      if (sfnSfIt != frameIt->second.end())
      {
        continue;
      }
      else
      {
        isPlaceholder = true;
        m_mapOfCSIRSResources.at(imsi).at(frameRemainder).insert (frameIt->second.begin() + (n % maxCSIperFrame), {{csiRSResourceSlot.GetSubframe (), csiRSResourceSlot.GetSlot ()}, isPlaceholder});
      }
    }
    else
    {
