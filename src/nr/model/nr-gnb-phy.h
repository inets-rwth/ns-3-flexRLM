...
public:
  ...
  /**
   * \brief Add the UE to the list of this gnbUEs
   * 
   * 
   * This one is not called from helper. Helper sets up m_ueAttachedImsiMap,
   * but m_deviceMap and m_ueAttached are empty initially, and will be filled 
   * only after a successful IA or Handover
   */
  ...
  void DoUpdateBeamsTBRLM (uint64_t imsi, std::vector<uint16_t> optimalBeamIndex, SfnSf startingSfn, std::vector<uint8_t> csiCounterVector);
...
protected:
  ...
  void virtual DoInitialize (void) override;
...
private:
  ...
  void QueueCSIRS (bool pushFront, uint64_t imsi);
