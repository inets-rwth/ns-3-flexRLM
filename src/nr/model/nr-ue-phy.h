...
public:
  ...
  void RegisterOtherEnb (uint16_t cellId, Ptr<NrGnbNetDevice> enb);

  /**
   * @brief Set the Phy Ideal Beamforming Helper object
   * 
   */
  ...
  void RegisterToRxSpectrum ();

  const Ptr<NetDevice> DoGetDevice ();

  virtual void SetPHYEpcHelper (Ptr<EpcHelper> epcHelper) override;
...
private:
  ...
  BeamId RetrieveNextRxBeamId (BeamId prevRxBeamId);
  ...
  void ReAdjustUEBeamToOperating ();
  ...
  //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  std::map<uint16_t, Ptr<NrGnbNetDevice> > m_registeredEnb;
  //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  std::pair<uint8_t, BeamId> m_lastOptimalCellBeamPair; // Cell-Beam pair that was obtained from the latest beam sweep
  ...
  std::map<uint8_t, BeamId> m_tempBeamIdStorageEnb;
  std::map<uint8_t, std::vector<std::pair<uint8_t, uint8_t>>> m_csiRSResourcesTbRLM;
  uint16_t m_noOfCSIRSResourcesPerSSBurst;
  std::vector<std::pair<uint8_t, std::pair<uint8_t, uint8_t>>> m_ssbRSResourceIndex;
  uint8_t m_ssbRMCounter {0};
  uint16_t m_noOfTxSSBScanDirections;
  int8_t m_startingIndex{-1};
