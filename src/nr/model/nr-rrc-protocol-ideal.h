...
private:
  ...
  void DoSendOptimalGnbBeamMap (LteRrcSap::CellOptimalGnbBeamMap msg);
  void DoTriggerRegisterUe (uint64_t imsi, const Ptr<NetDevice> &netDev);
  void DoSendDeregisterUe (uint64_t imsi);
  bool DoSendCheckForHandoverEvents (uint64_t imsi);
  void DoSendClearHandoverEvents (uint64_t imsi);
  void DoSendBeamPerformComplete (uint64_t imsi, bool beamSweepComplete);
  void DoSendSSBRSReport (LteRrcSap::UpdateBeamsTbRLM params);
  /**
  ...
  /// Function similar to above. It is run only when a IA has been performed 
  /// It would return true when a temporary EnbRrcSapProvider is to be used
  /// True would thus be the case that either UE was not connected to a cell before, or
  /// a new cell has been found as a result of the IA 
  /// False would indicate that the previous EnbRrcSapProvider will be used.
  bool SetTempEnbRrcSapProvider ();

  void SetLteEnbRrcSapProvider ();
  ...
  LteEnbRrcSapProvider* m_tempEnbRrcSapProvider;
  LteEnbRrcSapProvider* m_lteEnbRrcSapProvider;
...
private:
  ...
  void DoSendBeamSweepTriggerFromLTECoordinator (uint16_t rnti);
  void DoSendBeamSweepCommandFromLTECoordinator (uint16_t rnti);
