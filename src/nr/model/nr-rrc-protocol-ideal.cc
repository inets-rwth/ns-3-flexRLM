...
nrUeRrcProtocolIdeal::DoSendRrcConnectionReconfigurationCompleted (LteRrcSap::RrcConnectionReconfigurationCompleted msg)
{
  ...
  m_rnti = m_rrc->GetRnti ();
  SetEnbRrcSapProvider ();
  m_enbRrcSapProvider->RecvRegisterUeFromRRC (m_rrc->GetImsi ());
...
void 
nrUeRrcProtocolIdeal::DoSendOptimalGnbBeamMap (LteRrcSap::CellOptimalGnbBeamMap msg)
{
  NS_LOG_FUNCTION (this);

  Simulator::Schedule (RRC_IDEAL_MSG_DELAY,
                       &LteEnbRrcSapProvider::RecvOptimalGnbBeamMap,
                       m_lteEnbRrcSapProvider,
                       msg);
...
nrUeRrcProtocolIdeal::DoTriggerRegisterUe (uint64_t imsi, const Ptr<NetDevice> &netDev)
{
  ...
  m_enbRrcSapProvider->RecvTriggerRegisterUe (imsi, netDev);
}

void 
nrUeRrcProtocolIdeal::DoSendDeregisterUe (uint64_t imsi)
{
  LteRrcSap::DeRegisterUeContext params;
  params.imsi = imsi;
  params.cellId = m_rrc->GetCellId ();
  params.m_sourceOfCommand = LteRrcSap::DeRegisterUeContext::SourceOfCommand::DeRegisterFromUE;
  m_lteEnbRrcSapProvider->RecvDeRegisterUeCommand(params);
  /*Simulator::Schedule (RRC_IDEAL_MSG_DELAY,
                       &LteEnbRrcSapProvider::RecvDeRegisterUeCommand,
                       m_lteEnbRrcSapProvider,
                       params);*/
}

bool 
nrUeRrcProtocolIdeal::DoSendCheckForHandoverEvents (uint64_t imsi)
{
  return m_lteEnbRrcSapProvider->CheckForHandoverEvents (imsi);
}

void 
nrUeRrcProtocolIdeal::DoSendClearHandoverEvents (uint64_t imsi)
{
  m_lteEnbRrcSapProvider->RecvClearHandoverEvents (imsi);
}

void 
nrUeRrcProtocolIdeal::DoSendBeamPerformComplete (uint64_t imsi, bool beamSweepComplete)
{
  m_lteEnbRrcSapProvider->RecvBeamPerformComplete (imsi, beamSweepComplete);
}
...
void
nrUeRrcProtocolIdeal::SetLteEnbRrcSapProvider ()
{
  NS_LOG_FUNCTION (this);

  uint8_t lteCellId = 11;
  ...
  m_lteEnbRrcSapProvider = enbDev->GetRrc ()->GetLteEnbRrcSapProvider ();  
...
bool 
nrUeRrcProtocolIdeal::SetTempEnbRrcSapProvider ()
{
  /// This corresponds (probably) to the initiial case,
  /// where UE was not connected to any other cell before

  uint8_t bwpId;
  if (m_rrc->GetCellId () == m_rrc->GetTempCellId ())     
    {
      return false;
    }
  else
  {
    bwpId = m_rrc->GetTempCellId ();
    NS_ASSERT_MSG (bwpId != 0, "Temporary cell ID cannot be equal to zero");
    ...
    NS_ASSERT_MSG (found, " Unable to find gNB with BwpID =" << bwpId);
    m_tempEnbRrcSapProvider = gnbDev->GetRrc ()->GetLteEnbRrcSapProvider ();
    return true;
  }
...
void 
nrUeRrcProtocolIdeal::DoSendSSBRSReport (LteRrcSap::UpdateBeamsTbRLM params)
{
  Simulator::Schedule (RRC_IDEAL_MSG_DELAY,
                      &LteEnbRrcSapProvider::ForwardSSBRSBeamReport,
                      m_lteEnbRrcSapProvider,
                      params);
}
...
void 
NrGnbRrcProtocolIdeal::DoSendBeamSweepTriggerFromLTECoordinator (uint16_t rnti)
{
  // DOES NOTHING FOR NRGNBRRCPROTOCOLIDEAL
}


void 
NrGnbRrcProtocolIdeal::DoSendBeamSweepCommandFromLTECoordinator (uint16_t rnti)
{
  // DOES NOTHING FOR NRGNBRRCPROTOCOLIDEAL
}

void 
NrGnbRrcProtocolIdeal::DoSendUeDeRegistrationCompletedUpdate (uint16_t rnti, uint64_t imsi)
{
  // DOES NOTHING FOR NRGNBRRCPROTOCOLIDEAL
}
