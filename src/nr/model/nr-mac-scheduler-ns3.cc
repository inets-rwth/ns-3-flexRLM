      ...
      for (const auto &ue : beam.second)
        {
          if (ue.first->m_dlRBG == 0)
            {
              //maybe needed adjust assignment of RBG
...
//***********************************************************************************************
//The implementation in the mac scheduler is only for LTE fallback considerations, since no packets are scheduled and thus no traces can be generated it shows "holes" in the data
void
NrMacSchedulerNs3::IAdelay (Time delay)
{
  // function not used anymore
  NS_FATAL_ERROR("Function should not be used anymore");
  //IAisPerformed = true;
  NS_LOG_UNCOND ("IA triggered in scheduler -> delay applied at " << Simulator::Now().GetSeconds());
  Simulator::Schedule(delay, &NrMacSchedulerNs3::ResetIaDelayTrigger, this);
}

void
NrMacSchedulerNs3::ResetIaDelayTrigger()
{
  // function not used anymore
  NS_FATAL_ERROR("Function should not be used anymore");
  //IAisPerformed = false;
	NS_LOG_UNCOND("IA finished at " << Simulator::Now().GetSeconds());
}
...
NrMacSchedulerNs3::FormSSBlock (bool hasAccompanyingData, PointInFTPlane *startingPoint, SlotAllocInfo *slotAlloc, uint8_t availSym)
{
  if (hasAccompanyingData)
  {
    std::shared_ptr<DciInfoElementTdma> ssbDciInfo = std::make_shared<DciInfoElementTdma>
                ...
                std::vector<uint8_t> (GetBandwidthInRbg (),1));
  ...
  else
  {
    ...
    std::shared_ptr<DciInfoElementTdma> emptySlotDciInfo = std::make_shared<DciInfoElementTdma>
                ...
                std::vector<uint8_t> (GetBandwidthInRbg (), 0));
