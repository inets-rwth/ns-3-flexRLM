...
NrUeMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrUeMac")
    ...
    .AddTraceSource ("UeMacRxedPacketTrace",
                     "Ue MAC Rx Packet Throughput Traces.",
                     MakeTraceSourceAccessor (&NrUeMac::m_macRxedPacketTrace),
                     "ns3::MmWaveMacRxTrace::RxedUeMacPacketTracedCallback")
    .AddTraceSource ("UeMacTxedPacketTrace",
                     "Ue MAC Tx Packet Throughput Traces.",
                     MakeTraceSourceAccessor (&NrUeMac::m_macTxedPacketTrace),
                     "ns3::MmWaveMacRxTrace::TxedUeMacPacketTracedCallback")
  ;
  return tid;
}
...
NrUeMac::DoTransmitPdu (LteMacSapProvider::TransmitPduParameters params)
{
  ...
  m_macTxedPacketTrace(params.rnti, GetBwpId (), params.pdu->GetSize ());
...
NrUeMac::DoReceivePhyPdu (Ptr<Packet> p)
{
  ...
  m_macRxedPacketTrace(m_rnti,GetBwpId (), p->GetSize (), m_currentSlot, m_imsi);
  NrMacHeaderVs header;
