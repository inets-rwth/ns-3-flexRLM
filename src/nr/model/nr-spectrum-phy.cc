...
NrSpectrumPhy::StartRx (Ptr<SpectrumSignalParameters> params)
{
  ...
  Ptr<NrUeNetDevice> ueNetDevice = DynamicCast<NrUeNetDevice> (m_device);
  ...
  {
    if (ueNetDevice && nrDataRxParams->imsi != ueNetDevice->GetImsi())
    {
      // Any data transmission is send to every UE due to multi-model-spectrum-channel imlementation.
      // Ignore everything that is not for this UE!
      NS_LOG_INFO(" Received DATA not destined for this UE");
      return;
    }
    else
    {
      ...
      {
        NS_LOG_INFO(" Received DATA not in sync with this signal (cellId=" << nrDataRxParams->cellId << ", m_cellId=" << GetCellId() << ")");
      }
    }
  }
    ...
    if (!IsEnb())
    {
      double antennaGainLinear = std::pow(10.0,
                                          (dlCtrlRxParams->txAntennaGain +
                                           (ConstCast<ThreeGppAntennaArrayModel>(GetAntennaArray()))->GetGainDb()) /
                                              10.0);
      ...
      {
        NS_LOG_INFO("Received DL CTRL, but not in sync with this signal (cellId=" << dlCtrlRxParams->cellId << ", m_cellId=" << GetCellId() << ")");
      }
    }
    else
    {
  ...
  else if (ulCtrlRxParams != nullptr)
  {
    if (IsEnb()) // only gNBs should enter into reception of UL CTRL signals
    {
      ...
      {
        NS_LOG_INFO("Received UL CTRL, but not in sync with this signal (cellId=" << ulCtrlRxParams->cellId << ", m_cellId=" << GetCellId() << ")");
      }
    }
    else
    {
...
NrSpectrumPhy::StartTxDataFrames (const Ptr<PacketBurst>& pb, const std::list<Ptr<NrControlMessage> >& ctrlMsgList,
                                      Time duration, uint8_t slotInd, uint64_t imsi)
{
  NS_LOG_FUNCTION (this);
  switch (m_state)
    {
                                           ...
                                           const Time &duration)
{
  ...
  //auto containsSSB = false;
  //auto containsCSI = false;
    ...
    case IDLE:
      {
        ...
        txParams->pss = true;
        txParams->ctrlMsgList = ctrlMsgList;
        txParams->txAntennaGain = (ConstCast<ThreeGppAntennaArrayModel> (GetAntennaArray ()))->GetGainDb ();
        txParams->isSSB = false;
        txParams->isCSIRS = false;
        ...
        for (std::list<Ptr<NrControlMessage> >::const_iterator cMesIt = ctrlMsgList.begin (); cMesIt != ctrlMsgList.end(); cMesIt++)
        {
          ...
          if (msg->GetMessageType () == NrControlMessage::CSI_RS)
          {
            txParams->isCSIRS = true;
          }
        }
...
NrSpectrumPhy::StartRxDlCtrl (const Ptr<NrSpectrumSignalParametersDlCtrlFrame>& params)
{
  ...
  // or from other cells if the DL CTRL message is an SSB/CSI-RS,
  // all other CTRL from other cellIds will be ignored
  NS_LOG_FUNCTION (this);
  if ((params->cellId == GetCellId () && !IsEnb ()) || params->isSSB || params->isCSIRS)
  {
    if (params->isCSIRS)
    {
      Ptr<NrUeNetDevice> ueNetDevice = DynamicCast<NrUeNetDevice>(m_device);
      Ptr<NrCSIRSMessage> csiRSMsg = DynamicCast<NrCSIRSMessage>(params->ctrlMsgList.front());
      NS_ASSERT_MSG(csiRSMsg != 0, "CSI-RS message is not at params->ctrlMsgList.front()");
      if (ueNetDevice->GetImsi() != csiRSMsg->GetDestinationImsi())
      {
        return;
      }
    }
  ...
  for (auto &tbIt : m_transportBlocks)
    {
      ...
      GetTBInfo(tbIt).m_sinrAvg *= (ConstCast<ThreeGppAntennaArrayModel> (GetAntennaArray ())->GetGainDb ()) * 2;
