...
public:
  ...
  */
  ...
  void DoSetCSIRSFlag (uint64_t imsi, bool csiRSFlag);
...
private:
  ...
  std::pair<std::pair<bool, bool>, std::pair<uint64_t, uint8_t>> IsCSIRSRequired (const SfnSf &dlSfn);

  void DoSetCSIRSRLMResources (uint16_t csiRSPeriodicity, uint16_t csiRSOffset, uint8_t numOfBeamsTbRlm, uint64_t imsi, std::vector<uint8_t> csiCounter, bool isServingCellId);

  std::pair<uint8_t, uint8_t> DoGetInitialCSIRSResource (uint64_t imsi);

  void DoSetMACCSIRSTimer (bool csiRSTimer);

  void DoUpdateCSIRSRLMResources (uint64_t imsi, uint8_t maxCSIperFrame);
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  std::map<uint64_t, bool> m_csiRSFlag;
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  std::map<uint64_t,std::map<uint8_t, std::vector<std::pair<std::pair<uint8_t, uint8_t>, bool>>>> m_mapOfCSIRSResources;
  uint8_t m_noOfBeamsTbRLM;
  uint16_t m_csiPeriodicityInSlots;
  uint16_t m_csiOffsetInSlots;
  uint8_t m_minCSIRSFromServingGnb;
};
