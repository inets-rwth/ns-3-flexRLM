                      ...
                      Ptr<Ipv4Route> route)
{
  ...
  // add IP layer tx timestamp tag for latency logging
  IpTimestampTag ipTimestampTag;
  if(!(packet->PeekPacketTag (ipTimestampTag)))
  {
    ipTimestampTag.SetIpTimestamp (Simulator::Now().GetSeconds());
    packet->AddPacketTag (ipTimestampTag);
  }
